//
//  TestPreparationCustomCell.swift
//  ErnestinaClient
//
...
protocol TestPreparationCustomCellDelegate: class {
    func didChooseAnswer(btnIndex: Int)
}

class TestPreparationCustomCell: UICollectionViewCell {
    ...
    weak var delegate: TestPreparationCustomCellDelegate?
    
    var question: Question? {
        didSet {
            guard let unwrappedQue = question else { return }
            imgView.image = unwrappedQue.img
            ...
            if unwrappedQue.isAnswered {
                btnsArray[unwrappedQue.correctAns].backgroundColor=UIColor.green.withAlphaComponent(0.5)
                if unwrappedQue.wrongAns >= 0 {
                    btnsArray[unwrappedQue.wrongAns].backgroundColor=UIColor.red.withAlphaComponent(0.5)
                }
            }
        }
    }
    ...
    override func prepareForReuse() {
        btn1.backgroundColor=UIColor.black.withAlphaComponent(0.3)
        btn2.backgroundColor=UIColor.black.withAlphaComponent(0.3)
        btn3.backgroundColor=UIColor.black.withAlphaComponent(0.3)
        btn4.backgroundColor=UIColor.black.withAlphaComponent(0.3)
    }
    ...
    let imgView: UIImageView = {
        ...
        v.layer.borderColor = UIColor(white: 1.0, alpha: borderAlpha).cgColor
